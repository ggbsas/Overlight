# **Overlight v1.2**

Overlight is a Windows application that reduces screen brightness by applying a gamma adjustment. Designed for better eye comfort, especially in low-light environments, without the need to manually adjust your monitor’s hardware brightness.

> ⚠️ This application does **NOT** modify the physical monitor brightness or Windows display settings. Everything is handled in a visual and fully reversible way.

---

## 💡 Usage

- Launch the application
- Adjust the overlay darkness level using Hotkeys (Ctrl+Alt+Up/Down (Increase/Decrease), Ctrl+Alt+Esc (Exit))
- Use Left or Right click on the tray icon to open the brightness slider or access the context menu

The app runs in the system tray and remains active in the background until closed.

---

## ✨ Features

- Reduce brightness below your monitor’s minimum level
- Remembers last brightness level
- Low CPU and memory usage and no added input lag
- No driver or modify any system or hardware settings
- No administrator privileges required
- Overlight does not appear in screenshots
- Compatible with Windows 10/11 on any monitor

---

## 📦 Installation

### Option 1: Compiled

1. Download or clone this repository
2. Compile the main file 'Overlight.py'

### Option 2: Executable (Recommended)

1. If you don’t want to compile it, download the compiled executable here ⬇️

[![](https://img.shields.io/badge/Download-Overlight-purple?style=for-the-badge&labelColor=404040&color=8a2be2&logo=data:image/png;base64,)](https://drive.google.com/uc?export=download&id=1oyOWFiBmZUvZoOYq2k8JMghmru_jL4Rl)
